...
import {
  ...
  EventService,
  Spinner,
  CustomStampCreationInfo,
  customStampEvent,
  CustomStampEvent,
  CustomStampEventDetail,
  CustomStampService,
...
export class TsPdfViewer {
  ...
  constructor(options: TsPdfViewerOptions) {
    ...
    this._mainContainer = this._shadowRoot.querySelector(
      "div#main-container"
    ) as HTMLDivElement;

    this._eventService = new EventService(this._mainContainer);
    this._modeService = new ModeService({
    ...
    this._pageService = new PageService(
      this._eventService,
      this._modeService,
      this._docManagerService,
      { previewCanvasWidth: previewWidth, visibleAdjPages: visibleAdjPages }
    );

    this._customStampsService = new CustomStampService(
      this._mainContainer,
      this._eventService
    );
    ...
    this._previewer = new Previewer(
      this._pageService,
      this._shadowRoot.querySelector("#previewer")
    );
    this._viewer = new Viewer(
      this._modeService,
      this._pageService,
      this._shadowRoot.querySelector("#viewer"),
      { minScale: minScale, maxScale: maxScale }
    );
    this._viewer.container.addEventListener("contextmenu", (e) =>
      e.preventDefault()
    );
  ...
  protected onTextSelectionChange = () => {
    ...
    if (this._eventService.hasListenersForKey(textSelectionChangeEvent)) {
      ...
      this._eventService.dispatchEvent(
        new TextSelectionChangeEvent({ selectionInfos })
      );
    }
  };
  //#endregion
  ...
  private async openDocAsync(
    ...
    try {
      await this._docManagerService.openPdfAsync(
        type,
        src,
        fileName,
        this._userName,
        this.showPasswordDialogAsync,
        this.onPdfLoadingProgress
      );
    } catch (e) {
      throw e;
    } finally {
  ...
  private initMainContainerEventHandlers() {
    const mcResizeObserver = new ResizeObserver(
  ...
  private initViewControls() {
    ...
    this._shadowRoot
      .querySelector("#rotate-clockwise")
      .addEventListener("click", this.onRotateClockwiseClick);
    this._shadowRoot
      .querySelector("#rotate-counter-clockwise")
  ...
  private initFileButtons() {
    ...
    if (this._fileButtons.includes("open")) {
      this._fileInput = this._shadowRoot.getElementById(
        "open-file-input"
      ) as HTMLInputElement;
      this._fileInput.addEventListener("change", this.onFileInput);
      openButton.addEventListener(
        "click",
        this._fileOpenAction || this.onOpenFileButtonClick
      );
    } else {
      openButton.remove();
    }
    ...
    if (this._fileButtons.includes("save")) {
      saveButton.addEventListener(
        "click",
        this._fileSaveAction || this.onSaveFileButtonClickAsync
      );
    } else {
      saveButton.remove();
    }
    ...
    if (this._fileButtons.includes("close")) {
      closeButton.addEventListener(
        "click",
        this._fileCloseAction || this.onCloseFileButtonClick
      );
    } else {
      closeButton.remove();
    }

    const comparableOpenButton = this._shadowRoot.querySelector(
      "#button-command-comparison-open"
    );
    const comparableCloseButton = this._shadowRoot.querySelector(
      "#button-command-comparison-close"
    );

    if (this._comparableFileButtons.includes("open")) {
      this._comparableFileInput = this._shadowRoot.getElementById(
        "open-comparable-file-input"
      ) as HTMLInputElement;
      this._comparableFileInput.addEventListener(
        "change",
        this.onComparableFileInput
      );
      comparableOpenButton.addEventListener(
        "click",
    ...
    if (this._comparableFileButtons.includes("close")) {
      comparableCloseButton.addEventListener(
        "click",
  ...
  private onSaveFileButtonClickAsync = async () => {
    ...
    DomUtils.downloadFile(
      blob,
      this._docManagerService?.fileName ||
  ...
  private initModeSwitchButtons() {
    const modeButtons = this._shadowRoot.querySelectorAll(
      '*[id^="button-mode-"]'
    );
  ...
  private initAnnotationButtons() {
    // mode buttons
    this._shadowRoot
      .querySelectorAll('*[id^="button-annotation-mode-"]')
      .forEach((x) => {
  ...
  private onPaginatorChange = (event: Event) => {
    if (event.target instanceof HTMLInputElement) {
      const pageNumber = Math.max(
        Math.min(+event.target.value, this._docManagerService.pageCount),
        1
      );
  ...
  private moveToPrevPage() {
    ...
    const pageIndex = clamp(
      this._pageService.currentPageIndex - 1,
      0,
      this._pageService.length - 1
    );
  ...
  private onAnnotatorDataChanged = (event: AnnotatorDataChangeEvent) => {
    ...
    if (event.detail.saveable) {
      this._mainContainer.classList.add(
        event.detail.annotatorType + "-annotator-data-saveable"
      );
    }
    if (event.detail.undoable) {
      this._mainContainer.classList.add(
        event.detail.annotatorType + "-annotator-data-undoable"
      );
    }
    if (event.detail.clearable) {
      this._mainContainer.classList.add(
        event.detail.annotatorType + "-annotator-data-clearable"
      );
    }
  };
  ...
  private setAnnotationMode(mode: AnnotatorServiceMode) {
    ...
    this._shadowRoot
      .querySelector(`#button-annotation-mode-${prevMode}`)
      ?.classList.remove("on");
    this._shadowRoot
      .querySelector(`#button-annotation-mode-${mode}`)
      ?.classList.add("on");
  ...
  private onAnnotationModeButtonClick = (e: Event) => {
    const parentButton = (<Element>e.target).closest(
      '*[id^="button-annotation-mode-"]'
    );
    if (!parentButton) {
      return;
    }
    const mode = /button-annotation-mode-(.+)/.exec(
      parentButton.id
    )[1] as AnnotatorServiceMode;
  ...
  private onMainContainerPointerMove = (event: PointerEvent) => {
    ...
    const {
      ...
      width,
      height,
    } = this._mainContainer.getBoundingClientRect();
  ...
  //#region misc
  private onPdfLoadingProgress = (progressData: {
    loaded: number;
    total: number;
  }) => {
  ...
  private async refreshPagesAsync(): Promise<void> {
    ...
    this._shadowRoot.getElementById("paginator-total").innerHTML =
      docPagesNumber + "";
  ...
  private togglePreviewer() {
    if (this._previewer.hidden) {
      ...
      this._shadowRoot
        .querySelector("div#toggle-previewer")
        .classList.add("on");
