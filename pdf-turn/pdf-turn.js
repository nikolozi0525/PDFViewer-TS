...
var bookFlip = {
  ...
  init: function () {
    ...
    $(document).on("scrollmodechanged", () => {
      ...
      var button =
        PDFViewerApplication.appConfig.secondaryToolbar.bookFlipButton;
  ...
  start: function () {
    ...
    $("#viewer .page").each(function () {
      ...
      parent._height[$(this).attr("data-page-number")] =
        $(this).height() / scale;
    });

    $("#viewer")
      .removeClass("pdfViewer")
      .addClass("bookViewer")
      .css({ opacity: 1 });
    ...
    for (var page = 3; page < pages + (pages % 2); page++) {
      if (
        this._height[page] != this._height[page - 1] ||
        this._width[page] != this._width[page - 1]
      ) {
  ...
  stop: function () {
    ...
    $("#viewer .page").each(function () {
      ...
      $(this)
        .css("width", parent._size(page, "width"))
        .css("height", parent._size(page, "height"));
    });
  },
  ...
  load: function () {
    ...
    var max = Math.min(
      page + (this._spread === 0 ? 1 : 3 - (page % 2)),
      views.length
    );
